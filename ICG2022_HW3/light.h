...
public:
	...
	SpotLight() {
		...
		CutoffStart = glm::cos(glm::radians(30.0f));
		totalWidth = glm::cos(glm::radians(60.0f));
		CreateVisGeometry();
	}
	SpotLight(const glm::vec3 p, const glm::vec3 I, const glm::vec3 D, const float cutoffDeg, const float totalWidthDeg) {
		...
		intensity = I;
		direction = D;
		CutoffStart = glm::cos(glm::radians(cutoffDeg));
		totalWidth = glm::cos(glm::radians(totalWidthDeg));
		CreateVisGeometry();
	}
	// -------------------------------------------------------
